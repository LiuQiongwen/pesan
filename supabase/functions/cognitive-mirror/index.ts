...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Hard limit: 15 notes, title only (no summaries)
    const noteList = (notes as {title:string,tags:string[]}[])
      .slice(0, 15)
      .map(n => `- "${n.title.slice(0,50)}" [${(n.tags||[]).slice(0,3).join(",")}]`)
      .join("\n");

    const system = `You are a cognitive analysis engine. Analyze note titles/tags to identify HOW this person thinks.

Output ONLY valid compact JSON (no extra whitespace):
...
Rules: dominant_themes max 3 items. blind_spots max 2. bias_signatures max 2. stagnation_alerts max 1. Output JSON only.`;
