...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const system = `You are a semantic memory engine. Given a current note and a list of older notes, identify the 2-3 most conceptually relevant older notes — not just by tags, but by structural similarity of ideas.

For each related note, provide:
- id (exact match from input)
- reason: one sentence explaining why it's relevant (focus on the conceptual bridge, not just shared words)
...
If nothing is genuinely relevant, return [].`;
    ...
    const r = await fetch("https://api.enter.pro/code/api/v1/ai/messages", {
      ...
      body: JSON.stringify({ model: "anthropic/claude-sonnet-4.5", system, messages: [{ role: "user", content: userMsg }], stream: false, max_tokens: 500 }),
    });
