...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: ftsResults } = await db
      ...
      .eq("user_id", user_id)
      .textSearch("search_vector", query.replace(/[^\w\s\u4e00-\u9fff]/g, " ").trim(), {
        type: "plain",
        config: "simple",
      })
      .limit(20);

    // ── Step 2: Fallback — recent chunks if FTS returns nothing ───────────
    ...
    // ── Step 3: Enrich with note titles ───────────────────────────────────
    ...
    if (noteIds.length > 0) {
      ...
      for (const n of (notes || [])) {
        noteTitles[n.id] = n.title || "Untitled";
      }
    }
    ...
    if (candidates.length === 0) {
      const answer = "知识库中暂无相关内容。请先分析一些笔记，然后再搜索。\n\nNo relevant content found in your knowledge base. Please analyze some notes first.";
      return new Response(JSON.stringify({ success: true, answer, citations: [], conversation_id: null }), {
        headers: { ...cors, "Content-Type": "application/json" },
      });
    }

    // ── Step 4: Claude rerank + answer ────────────────────────────────────
    const contextBlocks = candidates.map((c: {id: string, content: string, source_title: string}, i: number) =>
...
6. Return ONLY valid JSON: {"answer": "text with [1] citations", "used_chunks": [1, 2, 3]}`;
    ...
    const { data: convo } = await db
      ...
      .insert({ user_id, project_id: project_id || "default", query, answer, citations })
      .select("id")
      .maybeSingle();

    return new Response(
      JSON.stringify({ success: true, answer, citations, conversation_id: convo?.id || null }),
      { headers: { ...cors, "Content-Type": "application/json" } }
    );
  } catch (e) {
