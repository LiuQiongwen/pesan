...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (candidates.length === 0) {
      return new Response(JSON.stringify({ success: true, answer: "知识库中暂无相关内容。请先分析一些笔记，然后再搜索。", citations: [], conversation_id: null }), { headers: { ...cors, "Content-Type": "application/json", "Expect": "" } });
    }
    ...
    return new Response(JSON.stringify({ success: true, answer, citations, conversation_id: convo?.id||null }), { headers: { ...cors, "Content-Type": "application/json", "Expect": "" } });
  } catch(e) {
