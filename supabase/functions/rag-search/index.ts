...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (candidates.length === 0) {
      return new Response(JSON.stringify({ success: true, answer: "知识库中暂无相关内容。请先分析一些笔记，然后再搜索。", citations: [], conversation_id: null }), {
...
6. Return ONLY valid JSON: {"answer": "text with [1] citations", "used_chunks": [1, 2, 3]}`;
    ...
    const citations = candidates
      .map((c: {id: string, note_id: string, source_title: string, content: string}, i: number) => ({
        id: i + 1, chunk_id: c.id, note_id: c.note_id, note_title: c.source_title,
