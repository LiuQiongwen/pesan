...
import {
  Check, X, RefreshCw, Filter, ArrowLeft,
  User, ChevronDown, ChevronUp,
  Upload, QrCode, Settings, TrendingUp, DollarSign, Clock, Package,
} from 'lucide-react';
    ...
    <div style={{
      ...
      >
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 2, minWidth: 90 }}>
          ...
          {order.user_email && (
            <span style={{ fontFamily: MONO, fontSize: 9, color: 'rgba(100,110,140,0.50)', paddingLeft: 16 }}>
              {order.user_email}
            </span>
          )}
        </div>
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .then(({ data }) => {
        ...
        const s: StatsData = {
          submitted: 0, fulfilled: 0, rejected: 0, pending: 0,
          totalRevenueFen: 0, todayNew: 0,
          subscriptionCount: 0, creditsCount: 0,
        };
        data.forEach(row => {
        ...
        {cards.map(c => (
          <div key={c.label} style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 8 }}>
              ...
              <span style={{ fontFamily: MONO, fontSize: 8, color: `${c.color}88`, letterSpacing: '0.06em' }}>
                {c.label}
              </span>
            </div>
            <div style={{ fontFamily: MONO, fontSize: 20, fontWeight: 700, color: c.color, letterSpacing: '-0.01em' }}>
              {c.value}
            </div>
          </div>
        ))}
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              ...
              <span style={{ fontFamily: MONO, fontSize: 9, color: 'rgba(100,110,140,0.60)', letterSpacing: '0.06em' }}>
                订单类型分布 · 共 {total} 笔
              </span>
            </div>
            <div style={{ display: 'flex', gap: 14 }}>
              <span style={{ fontFamily: MONO, fontSize: 9, color: '#b496ff' }}>
                订阅 {stats.subscriptionCount}
              </span>
              <span style={{ fontFamily: MONO, fontSize: 9, color: '#ffa040' }}>
                Credits {stats.creditsCount}
              </span>
            </div>
          </div>
